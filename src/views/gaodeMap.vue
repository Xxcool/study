  ...
  methods: {
    loadmap() {
      ...
      return new Promise((reslove, reject) => {
        AMapLoader.load({
          key: "36ec2106b9501df25e506dca3615cff4", // 申请好的Web端开发者Key，首次调用 load 时必填
    ...
    handleStartSelect(event) {
      ...
      this.insureAdress =
        event.poi.district + event.poi.address + event.poi.name;
      this.insureAdress2 =
        "[" + event.poi.location.lng + "," + event.poi.location.lat + "]";
      this.marker = new AMap.Marker({
        position: this.startSeacrh, //位置
      });
      console.log(this.startSeacrh,'---')
      this.map.add(this.marker); //添加到地图
      // 传入经纬度，设置地图中心点
