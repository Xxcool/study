<template>
  <div class="transfer">
    <div class="transfer-panel">
      ...
      <div v-if="showQuery">
        <el-form :inline="true" :model="leftQueryCondition" class="query-form">
          ...
          <el-form-item>
            <el-button type="primary" @click="onLeftQuerySubmit()">{{
              queryTexts[0]
            }}</el-button>
          </el-form-item>
        </el-form>
    ...
    <div class="transfer-buttons">
      ...
      >
        <span v-if="buttonTexts[0] !== undefined" class="button-text">{{
          buttonTexts[0]
        }}</span>
      ...
      >
        ...
        <span v-if="buttonTexts[1] !== undefined" class="button-text">{{
          buttonTexts[1]
        }}</span>
      </el-button>
    </div>
    <div class="transfer-panel">
      ...
      <div v-if="showQuery">
        <el-form :inline="true" :model="rightQueryCondition" class="query-form">
          ...
          <el-form-item>
            <el-button type="primary" @click="onRightQuerySubmit()">{{
              queryTexts[1]
            }}</el-button>
          </el-form-item>
        </el-form>
...
export default {
  ...
  computed: {
    ...
    disabledLeftButton () {
      return !this.leftSelection.some(
        (leftRow) =>
          !this.rightTableData.some((rightRow) =>
            this.checkObjectIsEqual(leftRow, rightRow)
          )
      )
    },
    calcRightTableData () {
  ...
  methods: {
    ...
    handleLeftRowClick (row) {
      if (
        !this.rightTableData.some((rightRow) =>
          this.checkObjectIsEqual(rightRow, row)
        )
      ) {
        ...
        this.paginationCallBack.call(null, condition).then((result) => {
          ...
          this.$nextTick(() => {
            this.leftTableData.forEach((leftRow) => {
              const isHave = this.rightTableData.some((rightRow) =>
                this.checkObjectIsEqual(rightRow, leftRow)
              )
    ...
    handleSelectable (row) {
      return !this.rightTableData.some((rightRow) =>
        this.checkObjectIsEqual(rightRow, row)
      )
    },
    addToRight () {
      for (const item of this.leftSelection) {
        const isHave = this.rightTableData.some((rightRow) =>
          this.checkObjectIsEqual(rightRow, item)
        )
        if (!isHave) {
    ...
    addToLeft () {
      this.rightSelection.forEach((item) => {
        const index = this.rightTableData.findIndex((rightRow) =>
          this.checkObjectIsEqual(rightRow, item)
        )
        if (index !== -1) {
          this.rightTableData.splice(index, 1)

          const leftRow = this.leftTableData.find((leftRow) =>
            this.checkObjectIsEqual(leftRow, item)
          )
          if (leftRow) {
    ...
    onRightQuerySubmit () {
      this.rightConditionTemp = JSON.parse(
        JSON.stringify(this.rightQueryCondition)
      )
    },
